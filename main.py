...
class YouTubeDownloaderApp(MDApp):
    ...
    def on_activity_result(self, requestCode, resultCode, intent):
        ...
        try:
            ...
            uri = intent.getData()
            content_resolver = autoclass('android.context.ContentResolver')
            cr = autoclass('org.kivy.android.PythonActivity').mActivity.getContentResolver()
            cr.takePersistableUriPermission(uri, 0x3)  # READ | WRITE
    
            # Get path from URI (display name)
            document_file = DocumentFile.fromTreeUri(autoclass('org.kivy.android.PythonActivity').mActivity, uri)
            display_name = document_file.getName() or "Selected Folder"
    
            # Store URI for later use
            self.download_folder_uri = uri.toString()
            self.download_folder = display_name  # just for display
            self.root.ids.folder_label.text = display_name
    
            self._show_dialog(f"Folder selected: {display_name}")
    ...
    # REPLACE your open_folder_picker + open_file_manager with this:
    def open_file_manager(self):
        if platform == 'android':
            try:
                from jnius import autoclass
    ...
    def _download_thread(self, raw_url: str):
        ...
        if platform == 'android' and hasattr(self, 'download_folder_uri'):
            try:
                from jnius import autoclass
                PythonActivity = autoclass('org.kivy.android.PythonActivity')
                uri = autoclass('android.net.Uri').parse(self.download_folder_uri)
                # We'll use yt-dlp with custom file access via URI (advanced)
                # For now, fallback to app-private dir
                out_dir = PythonActivity.getFilesDir().getAbsolutePath()
                outtmpl = os.path.join(out_dir, "%(title)s.%(ext)s")
            except:
                out_dir = "/sdcard/Download"  # fallback
                outtmpl = os.path.join(out_dir, "%(title)s.%(ext)s")
        else:
            outtmpl = os.path.join(self.download_folder, "%(title)s.%(ext)s")
